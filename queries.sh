...
echo  "$($PSQL "SELECT DISTINCT(name) FROM games g LEFT JOIN teams w ON g.winner_id = w.team_id OR g.opponent_id = w.team_id WHERE round = 'Eighth-Final' AND year = 2014 ORDER BY name ")"
